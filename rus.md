...
Сегодня вы можете задавать объекту любую [форму][1] CSS-трансформациями, но эти формы не могут влиять на поведение содержимого внутри них или вокруг них. Например, если вы создаете треугольник или трапецию с помощью CSS, то созданная форма не определяет и не влияет на то, как будет вести себя текст внутри фигуры или вне ее.
...
Когда я писала статью, только Chrome Canary [поддерживал версию CSS-форм без префиксов][3], но для того, чтобы они заработали, нужно включить [специальный флаг в настройках][4].
...
Все HTML-элементы имеют прямоугольную блочную модель, которая управляет потоком содержимого внутри и снаружи нее. Для того, чтобы придать элементу непрямоугольную форму, мы используем свойства `shape-inside` и `shape-outside`. Когда я писала эту статью, свойство `shape-outside` применялось только для плавающих элементов, а свойство `shape-inside` не было полностью внедрено, поэтому при использовании этих свойств у вас могут возникать ошибки. 
Свойства shape-* могут применяться только к блочным элементам. Неблочные элементы должны приводиться стилями к блочным, и только тогда можно использовать shape-свойства.

Shape-* свойства принимают одно из трех значений: auto, базовая фигура или URI изображения.
...
Если свойству задана «функция формы» (shape function), то форма вычисляется на основе выбора одного из значений функций - `rectangle`, `inset-rectangle`, `circle`, `ellipse`, `polygon`. Больше информации об этих функциях вы сможете найти в [статье][6] команды Adobe Platform.
...
>В то время как мы можем ограничивать строчный контент с помощью shapes (форм) реальный его контейнер не изменяется. Если элемент имеет конкретные внешние и внутренние отступы, границы, то они будут расчитаны в соотвествии с [CSS3BOX][7]. — [W3C CSS Shapes Module Level 1][8] 
...
_Фон применен к заданой пользователем форме_
...
На момент редактирования статьи свойство `shape-outside` работало только на плавающих элементах, а свойства `shape-outside` и `shape-inside` применялись только к блочным элементам или же к инлайновым элементам, *приведенным к блочным*.
Инлайновый контент элемента, определённого как плавающего, будет иметь форму фигуры, заданной этому элементу, а не границы элемента. Будущие уровни CSS-форм позволят использовать формы не только на плавающих элементах, и когда это случится, контент сможет обтекать элемент с двух сторон формы (как на картинке ниже). Так что пока мы можем только сделать элемент плавающим, и содержимое будет обтекать его с одной из сторон.

![Расположение контента с двух сторон от CSS-формы][13]  
_Расположение контента с двух сторон от CSS-формы_

Вы можете сэмулировать обтекание содержимого с двух сторон, используя 
...
_Скриншот демо#1. Нажмите на скриншот, чтобы увидеть работающее демо_
...
Сперва мы зададим плавающему элементу `div` конкретную высоту и ширину для установки системы координат. Мы установим высоту, равную высоте контейнера, которому для этого примера я задала высоту вьюпорта, используя единицу измерения `vh`.
...
В первом способе мы будем использовать функцию polygon(). Данная функция принимает набор точек, которые образуют прямоугольник; каждая точка задается через координаты `x` и `y`. Мы собираемся определить очень простую прямоугольную фигуру с четырьмя вершинами, как показано на рисунке ниже (оранжевые и голубые точки):

![Скриншот, на котором изображены вершины, которые образуют прямоугольную форму][18]  
_Скриншот, на котором изображены вершины, которые образуют прямоугольную форму_
...
Сейчас всё, что мы должны сделать, — это указать форму плавающему элементу так, чтобы текст обтекал его.
...
И это все! Теперь текст обтекает плавающий элемент по форме, которую мы определили.

Также вы можете видеть, что я добавила внешний отступ, чтобы отдалить содержимое от формы и создать промежуток между ними.
...
Второй способ определять формы на элементах – это использование изображений с альфа-каналом, то есть любых изображений с прозрачностью.
...
Каждый из указанных двух методов имеет свои преимущества. Изображения с альфа-каналом следует использовать для сложных фигур, где было бы затруднительно создавать точки вручную; в этом случае создание изображения в фотошопе будет намного проще и быстрее, чем добавление точек руками.
...
_Скриншот демо#2. Нажмите на скриншот, чтобы увидеть работающее демо_

Цель этого примера - показать использование свойства `shape-inside` для плавающего текста в непрямоугольной форме. У нас есть контейнер с несколькими заглушками текста внутри него, и мы применяем фоновое изображение к этому контейнеру.
...
Функция `circle()` имеет три аргумента: `cx`, `cy` и `r` (радиус), где `cx` и `cy` - координаты центра окружности, а `r` — радиус окружности.
...
![Система координат и форму определенные на контейнере][23]  
_Система координат и форму определенные на контейнере_

Так как мы хотим упаковать текст *внутрь* пользовательской формы, а не обтекать его вокруг, то мы будем применять свойство `shape-inside` к элементу, который содержит этот текст. Когда мы применяем свойство `shape-inside` к элементу, мы должны помнить, что текст должен находится внутри этого элемента, что не похоже на предыдущий пример, где текст был снаружи. 
...
![Изображение с альфа-каналом определяющее форму для демо#2][32]
Изображение с альфа-каналом определяющее форму для демо#2
...
В этом примере мы собираемся создать множественные области внутри элементов, в которые заключим контент. Результирующее демо показано на рисунке: 

![Скриншот демо#3][33]  
_Скриншот демо#3_

Мы имеем `div` с фоновым изображением, и мы хотим, чтобы текст внутри этого `div` находился внутри определенных областей, каждая из которых имеет свою форму.
...
![Изображение с альфа-каналом для определения формы для демо#3][34]  
_Изображение с альфа-каналом для определения формы для демо#3_

В определении элемента `.container` в значение свойста `shape-inside` мы подставим наше изображение с альфа-каналом и укажем блоку высоту и ширину:
...
Вот и всё. Оставшуюся работу за нас сделал браузер, достав формы из изображения, которое мы ему дали, и аккуратно расположив текст внутри этих форм.
...
В скором времени должна появиться широкая поддержка CSS-форм; команда веб-разработчиков из Adobe постоянно работает над улучшением и внедрением новых возможностей и создаёт инструменты для упрощения работы с ними. 

Будущее веб-разработки выглядит все ярче и привлекательней с каждым днем. Хорошо быть веб-разработчиком!

Я надеюсь, что эта статья поможет вам разобраться, как работать с CSS-формами и исключениями. Это не последняя моя статья на эту тему. Комбинирование CSS-форм с другими передовыми технологиями мира CSS, такими как регионы, открывает двери в новый мир творческих идей и создания новых статей! ;-)  
